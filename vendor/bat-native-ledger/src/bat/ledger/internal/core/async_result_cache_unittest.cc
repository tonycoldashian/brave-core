...
class AsyncResultCacheTest : public testing::Test {
 protected:
...
TEST_F(AsyncResultCacheTest, ReturnsPendingRequests) {
  ...
  cache.GetResult(generate_value)
...
TEST_F(AsyncResultCacheTest, ReturnsFreshResults) {
  AsyncResultCache<int> cache;
  ...
  cache.GetResult(generate_cache_value);
  EXPECT_EQ(generate_calls, 1);
...
TEST_F(AsyncResultCacheTest, SupportsUniqueKeys) {
  AsyncResultCache<int, int> cache;

  int generate_calls = 0;
  auto generate_value = MakeValueGenerator(&generate_calls);

  cache.GetResult(1, generate_value);
  cache.GetResult(2, generate_value);
